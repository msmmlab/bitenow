...
export function getIntentOptions(timeLens, date) {
    ...
    // For NOW and LATER, base it on the time of day
    const hour = date.getHours();

    if (hour >= 5 && hour < 10.5) return ["Coffee", "Breakfast"];
    if (hour >= 10.5 && hour < 14.5) return ["Lunch", "Coffee"];
    if (hour >= 14.5 && hour < 17) return ["Beer", "Coffee"];
    if (hour >= 17 && hour < 21) return ["Beer", "Dinner"];
...
export function scoreRestaurant(restaurant, context) {
    ...
    if (timeLens === "now") {
        ...
        // NOW Penalties: slow, expensive, booking-heavy
        ...
        // TONIGHT Adjustments: allow slower service, reduce price penalty
        if (restaurant.service_speed === "slow") score += 5;
        // Price penalty reduction handled in price_risk section below
    }
    ...
    if (intent === "Dinner") {
        if (bestTimes.includes("dinner")) score += 40;
