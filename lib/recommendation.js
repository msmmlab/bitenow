...
export function getMealBucket(date, timeLens) {
    ...
    if (adjustedTime >= 6.5 && adjustedTime < 10.5) return "breakfast";
    if (adjustedTime >= 10.5 && adjustedTime < 14.5) return "lunch";
...
export function getIntentOptions(timeLens, date) {
    ...
    if (timeLens === "later") {
        if (hour < 10) lookupHour = 12;      // Morning -> Lunch
        else if (hour < 14) lookupHour = 16; // Lunch -> Beer/Afternoon
        else lookupHour = 19;                // Afternoon -> Dinner
    }

    if (lookupHour >= 5 && lookupHour < 10.5) return ["Coffee", "Breakfast"];
    if (lookupHour >= 10.5 && lookupHour < 14.5) return ["Lunch", "Coffee"];
    if (lookupHour >= 14.5 && lookupHour < 17) return ["Beer", "Coffee"];
    if (lookupHour >= 17 && lookupHour < 21) return ["Beer", "Dinner"];
...
export function scoreRestaurant(restaurant, context) {
    ...
    if (timeLens === "tonight" || intent === "Fancy dinner") priceRiskPenalty = -5; // reduce penalty
    ...
    if (intent === "Beer") {
        ...
        if (isVibePub || bestTimes.includes("beer")) score += 40;
